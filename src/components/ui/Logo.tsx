...
export function LogoWordmark({ size = 28, className }: LogoWordmarkProps) {
  ...
  return (
    <div className={cn('flex items-center', className)} style={{ gap: Math.round(size * 0.34) }}>
      <LogoMark size={size} />
      <div className="flex items-baseline gap-1">
        <span
          className="font-semibold uppercase tracking-[-0.06em] text-foreground"
          style={{ fontSize, lineHeight: 1 }}
        >
          Arc
        </span>
        <span
          className="font-semibold uppercase tracking-[0.18em] text-white/[0.58]"
          style={{ fontSize: Math.max(11, Math.round(size * 0.38)), lineHeight: 1 }}
        >
          Comm
        </span>
      </div>
